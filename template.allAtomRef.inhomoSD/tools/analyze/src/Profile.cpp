	 ...
	 const std::vector<std::vector<double > > & values)
{
  for (unsigned i = 0; i < posis.size(); ++i){
    ...
    unsigned iz = unsigned (tmp / hz);
    profile[index3to1(ix, iy, iz)][0] += values[i][0];
    profile[index3to1(ix, iy, iz)][1] += values[i][1];
    profile[index3to1(ix, iy, iz)][2] += values[i][2];
    ndata[index3to1(ix, iy, iz)] += 1.;
  }  
}
...
print_x (const std::string & file) const 
{
  ...
  for (unsigned i = 0; i < nx; ++i){
    ...
    // fprintf (fp, "%f %f\n", (i + 0.5) * hx, sum / ny / nz);
    fprintf (fp, "%f %f %f %f\n", (i + 0.5) * hx,
	     profile[index3to1(i, ny/2, nz/2)][0],
	     profile[index3to1(i, ny/2, nz/2)][1],
	     profile[index3to1(i, ny/2, nz/2)][2]);
  }
...
print_xz (const std::string & file) const 
{
  ...
  for (unsigned i = 0; i < nx; ++i){
    ...
    for (unsigned j = 0; j < nz; ++j){
      ...
      for (unsigned k = 0; k < ny; ++k){
	unsigned index = index3to1(i, k, j);
      	sum[0] += profile[index][0] * ndata[index];
      	sum[1] += profile[index][1] * ndata[index];
      	sum[2] += profile[index][2] * ndata[index];
	nsum += ndata[index];
      }
      if (nsum != 0){
	sum[0] /= nsum;
