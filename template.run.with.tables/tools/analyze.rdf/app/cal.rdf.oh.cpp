...
int main(int argc, char * argv[])
{
  ...
  coms1.reserve (nmolecules);
  std::vector<std::vector<ValueType > > coms2;
  coms2.reserve (nmolecules*2);

  CellList clist1 (nmolecules, vbox, cellSize);
  CellList clist2 (nmolecules, vbox, cellSize);
  ...
  while (read_xtc (fp, natoms, &step, &time, box, xx, &prec) == 0){
    ...
    for (int i = 0; i < nmol; ++i){
      if      (xx[i*atomPerMol+0][0] <  0        ) xx[i*atomPerMol+0][0] += box[0][0];
      else if (xx[i*atomPerMol+0][0] >= box[0][0]) xx[i*atomPerMol+0][0] -= box[0][0];
      if      (xx[i*atomPerMol+0][1] <  0        ) xx[i*atomPerMol+0][1] += box[1][1];
      else if (xx[i*atomPerMol+0][1] >= box[1][1]) xx[i*atomPerMol+0][1] -= box[1][1];
      if      (xx[i*atomPerMol+0][2] <  0        ) xx[i*atomPerMol+0][2] += box[2][2];
      else if (xx[i*atomPerMol+0][2] >= box[2][2]) xx[i*atomPerMol+0][2] -= box[2][2];
      ...
      tmp[0] = xx[i*atomPerMol+0][0];
      tmp[1] = xx[i*atomPerMol+0][1];
      tmp[2] = xx[i*atomPerMol+0][2];
      coms1.push_back(tmp);
    ...
    clist1.rebuild (coms1);
    clist2.rebuild (coms2);
    myrdf.deposit (coms1, coms2, vbox, clist1, clist2);
  }
  printf ("\n");
