...
\begin{abstract}
  ...
  work done by \redc{the  transition region} is formally equivalent to balance the chemical
...
\end{enumerate}
\redc{The equilibrium in the AT and CG region is assured by the membrane $\HY$.} Conceptually, we assume there is an infinitely thin
...
regions is taken care by the work of the filter. \redc{Now if
the filter ensures and equilibrium
that is the same as the full atomistic reference system, then}
...
condition for \redc{the hybrid region} that ensures
the correct equilibrium of the AT and CG regions.\\

\noindent
\redc{
The work done by the filter is composed by two parts:
\begin{align}\label{eqn:w0-inte-qstat}
  \omega_0 = \omega^\dadress_\thf + Q_\thermo,
\end{align}
where $\omega_\thf$ is the integral of the thermodynamic force,
% (the superscript D indicate this the thermodynamic force of the
% force interpolation AdResS),
and $Q_\thermo$ is the extra heat provided
by the thermostat that equilibriates
the system. This term exists for two reasons,
firstly, the kinetic energy of a molecule is grows when it gains
DOFs, and the extra DOFs are warmed up by the thermostat.
Secondly, the force interpolation scheme is
not conservative, so there is an artificial energy generation in the
hybrid region, which is equilibriated by the thermostat~\cite{prlcomm}.
We will show in Appendix~\ref{app:0} that
$Q_\thermo = \omega_\dof + \omega_\res$, where $\omega_\dof$ is the
kinetic energy increment
due to the extra DOFs when a low resolution molecule gains
finer resolution. According to the equipartition theorem, this term
is equal to $\frac12 k_BT$ times the number of extra DOFs.
The energy of changing representation $\omega_\res$ is defined by
\begin{align}
  \omega_\res
  = \int\dd d\vect r\,\nabla w(\vect r)\,
  \big\langle w\, (U^\AT - U^\CG)\big\rangle_{\vect r}
\end{align}
where $\langle \cdot \rangle_{\vect r}$ is
the local ensemble average measured at position $\vect r$, and $U^\AT - U^\CG$
is the differece between the atomistic and coarse-grained intermolecular
interaction.
}
\\
...
\end{align}
This, in the previous section, has been shown to hold because of the action of the \redc{transition region} for the thermodynamic limit. 
...
Fig.~\ref{fig:wca-dist} shows the COM-COM, H-O and H-H
    ...
    is corrected by the \redc{thermodynamic force} in the $\HY$ region.
...
\redc{
  It is also possible to use the AdResS to measure the chmical potential.
  Since applying IPM is general much easier for the CG molecule, 
  by relation~\eqref{eqn:w-mu-diff} that gives the chemical potential
  difference between the AT and CG resolutions, the chemical potential of
  the AT resolution can be easily calculated.
  Since the chemical potential is the first order quantity of the
  system, we only consider the first order correction (the thermodynamic
  force without RDF correction) and the first order CG model --
  the WCA potential.
  For water system the resulting chemical potential is -22.7\,\textsf{kJ/mol}.
  Unfortunatly, we do not have the converged IPM result to compare,
  however, we find our result consists with the literature value
  -23.5\,\textsf{kJ/mol}~\cite{flo} very well.
  To be more sure, we also test the method in the modeling system:
  the AT molecules are modelled by spherical particles
  interacting via the $r^{-12}$ repulsive
  interaction, the CG molecules are modelled by the WCA potential.
  The AdResS measured chemical potential is 56.0\,\textsf{kJ/mol},
  which is consistent with
  the IPM  result: 55.4 \,\textsf{kJ/mol}.
}
...
Using strict formal arguments, we have demonstrated the \redc{work of the transition region as} a tool to balance the difference in chemical potential due to the different resolutions in space.
...
\section{Calculating the work of the thermostat}\label{app:0}
\redc{
  To calculate the work of the thermostat, it is convenient to use
  an auxilary Hamiltonian approach, 
where we consider the potential interpolation of the
...
\begin{align}
  \vect F_{\res,i} = 
  \sum_j \nabla w_i \,w_j (U^\AT_{ij} - U^\CG_{ij})
\end{align}
...
action-reaction law.
By using the same process as the force interpolation AdResS,
we can calculate the thermodynamic force for the
energy interpolation interpolation AdResS,
which is denoted by $\vect F_\thf^\hadress$
(the superscript H indicates this is the thermodynamic force of the
potential interpolation based AdResS). Numerical result shows
(not presented in the paper) that the energy interpolation AdResS
has the same equilibrium as the force interpolation AdResS
upto the first order (correct equilibrium density).
With the potential interpolation, we can write the global Hamiltonian
...
write the fixed-number partition function:
\begin{align}
  q(N, V, T)
  \approx
  \frac1{N!}\int\dd d\vect x_1\dd d\vect x_3\,
...
\end{align}
Since the system is energy conservative,
the work of the filter is identical to $\omega^\hadress_\thf+ \omega_\dof$,
whre $\omega^\hadress_\thf$ is the integral
of the thermodynamic force, and  $\omega_\dof$ is the extra
kinetic energy, when turning on extra degrees of freedom in
the AT region. For rigid water system, $\omega_\dof = \frac32
k_BT$. Following the same argument of the proof in
Sec.~\ref{sec:th-gc}, we have
...
region. Without lost of generality, we assume this
volume change is  along the $x$ axis.
The volume of the hybrid region is kept the same,
so it is a ``piston''
moves toward the CG region by $\Delta L$,
where we assume $\Delta V = \Delta L\cdot A$, where $A$ is the
cutting surface area (the size of box along $y$ times the size along $z$).
...
\begin{align}
  ...
  \int\dd d x\, \rho_0 A\cdot \Delta L\cdot
  \Big[
...
\begin{align}
  \omega_\res
  = \int\dd d x\,\langle \vect F_\res (x)\rangle
  = \int\dd d x\,\nabla\omega(x)\,
...
\end{align}
For the force interpolation AdResS, by definition Eq.~\eqref{thf},
we have
\begin{align}\label{eqn:peq-d}
...
\begin{align}\label{eqn:hd-rel}
  \omega_\thf^\dadress =  \omega_\thf^\hadress - \omega_\res 
\end{align}
In the water system considered by the present paper,
$\omega^\hadress_\thf - \omega^\dadress_\thf = 34.9 \,\textsf{kJ/mol}$, and
if measure in the energy interpolation AdResS simulation,
$\omega_\res = 35.0 \,\textsf{kJ/mol}$,
if measure in the force interpolation AdResS simulation,
$\omega_\res = 34.6 \,\textsf{kJ/mol}$.
...
interpolation AdResS. The consistence of the $\omega_\res$ measurement
also indicates this point. Therefore, we propose the relation between
the thermodynamic force of the energy interpolation AdResS and
force interpolation AdResS:
\begin{align}
  \vect F_\thf^\dadress \approx \vect F_\thf^\hadress - \vect F_\res
\end{align}
The numerical proof of this relation in the water system
is given in Fig.~\ref{fig:diff-thf}.
...
\begin{figure}
  \centering
  \includegraphics[]{diff-thf.eps}
  \caption{\redc{The difference between the thermodynamic force of the
    energy interpolation and force interpolation AdResS.
    The green solid line shows the difference of the thermodynamic
    forces, i.e. $\vect F_\thf^\hadress - \vect F_\thf^\dadress$.
    The red and blue cross indicates the work of changing
    representation. The former was measured in an energy interpolation
    AdResS, while the later was measured in a force interpolation
    AdResS.}
  }
  \label{fig:diff-thf}
\end{figure}
...
\noindent
In the  equilibrium of energy interpolation AdResS, the work
of the filter is $\omega_\thf^\hadress + \omega_\dof$. In the equilibrium
of the force interpolation AdResS,
by using relation Eq.~\ref{eqn:hd-rel},
the work of the filter is
$\omega_\thf^\dadress + Q_\thermo = \omega^\hadress_\thf + \omega_\dof
- \omega_\res -\omega_\dof + Q_\thermo$. Since the energy and force
interpolation reach the same thermodynamic equilibrium, the work
of the filter should be the same, this eventually indicates
\begin{align}
  Q_\thermo = \omega_\res + \omega_\dof.  
\end{align}
}
