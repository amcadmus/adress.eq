...
\end{align}
\redc{where the Hamiltonian of the atomistic system is defined as}:
...
\begin{figure}
  ...
  \caption{\redc{The 3-body correlation function $\corr$.  The 1st row corresponds to the reference all atom simulation.
...
Here we report the H-O and H-H RDFs (see Fig.~\ref{fig:tmp2a}), and the 3-body \redc{correlation function
$\corr(\vect r_1, \vect r_2, \vect r_2)$} (see Fig.~\ref{fig:tmp2b}). For simulation
protocols and the definition \redc{$\corr$}, see the Appendix~\ref{app:2} and \ref{app:3}.
...
\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{fig-rdf3-diff-wca-ex-hy1.eps}
...
(on average) 34 water molecules entered into the atomistic region. Some of
...
\begin{table}
  ...
  \end{tabular}
  \caption{\redc{CPU time.
    The AdResS simulation contains 13824 molecules, while the EX 
...
\begin{figure}
  ...
  \caption{\redc{The chemical potential calculated by the test particle insertion method. The top panel shows the case of coarse-grained water and WCA liquid while the bottom panel shows the case of a full atomistic simulation. The convergence of the chemical potential means that the insertion of the molecule is properly done and sudsequent part of the trajectory can be used for ``statistical'' average in a Grand Canonical framework.}}
  \label{fig:tmp4}
\end{figure}
...
distribution is simplified as follows. We first fix the distance between molecule 1 and 2 (Mol
