...
int main(int argc, char * argv[])
{
  ...
  desc.add_options()
      ...
      ("input-0",   po::value<std::string > (&i0file)->default_value ("rdf3.0.out"), "the input rdf3 file")
      ("input-1",   po::value<std::string > (&i1file)->default_value ("rdf3.1.out"), "the input rdf3 file")
      ("output,o",   po::value<std::string > (&ofile)->default_value ("rdf3.diff.out"), "the input rdf3 file");
  ...
  while (fp0.getline(nameline0, MaxLineLength)){
    ...
    if (ss0.size() == 0){
      ...
      double v0 = atof(ss0[2].c_str());
      double v1 = atof(ss1[2].c_str());
      fprintf (fp, "%s %s %.16e\n",
	       ss0[0].c_str(), ss0[1].c_str(), v0 - v1);
    }
  }
