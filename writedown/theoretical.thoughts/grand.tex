...
\begin{figure}
  ...
  \caption{The 3-body correlation function.  The 1st row:
    ...
    definition of these variables).  The maganitude that indicated by
    different colors presents the maganitude of $g^{(3)}$.
  }
  \label{fig:tmp2b}
\end{figure}


Up to now, it is still not sufficient to argue the probability
$p(\vect x_1 , N_1)$ is correct, namely is subject to the grand canonical
distribution 
Eqn.~\eqref{eqn:p}.
We are very careful about any statement on $p(\vect x_1, N_1)$ 
due to the following three reasons: 1,
The Boltzmann formulus of the distribution $p(\vect x_1|N_1; \vect x_2, N_2)$
is only an approximation. 2, Conditions \eqref{eqn:p-1-cond1} and \eqref{eqn:p-1-cond2}
are only necessary, and are generally not sufficient to guarentee
the right distribution $p(\vect x_2, N_2 | N_1)$ in the hybird region $\HY$. 3,
The number distribution $p(N_1)$ is only correct up to the second order.
Since the number distribution $p(N_1)$ has already
been numerically tested in Ref.~\cite{prlgc, rdfcorr},
in this section, we want to numerically test to what extent the configurational
distribution is correct. 
Notice that it is impossible to depict the high-dimensional configurational distribution,
so, instead, we study its marginal distribution upto the third order.
Here equivalently, we plot the H-O and H-H RDFs (see Fig.~\ref{fig:tmp2a}), and the 3-body COM
RDF $g^{(3)}(\vect r_1, \vect r_2, \vect r_2)$ (see Fig.~\ref{fig:tmp2b}). For simulation
protocols and the definition of 3-body RDF, please see the Appendix.
The density profile and the COM RDF are not presented here,
we refer the readers to Ref.~\cite{rdfcorr}. 
It can be seen from Fig.~\ref{fig:tmp2a} that
the AdResS H-O and H-H RDFs are identical with
the full atomistic reference system in the AT region.
More importantly, these RDFs correct
in the hybrid sub-region HY I that next to the AT region. Please notice that
the TFI-IBI iteration~\cite{rdfcorr} we used is designed only
to fit the COM RDF in the hybrid region, 
so there is actually \emph{no} guarentee that the orientational order of water, indicated
by H-O and H-H RDFs, are also correct. We are fortune to see the orientational
order are also preserved  in sub-region HY I, which means the AT region
is imbeded in an environment where not only the COM but also the orientational
pair structures are the same as the atomistic resolution.
In the HY II and III regions, the AdResS obviously deviate from the
full atomistic reference, because the atomistic nature of nature
is fading as a molecule travels from HY I to HY III, so we can imagine
that the orientational order is also fading. In HY III, the H-O and H-H
RDFs are acutually structureless.
Furthermore,
we test the 3-body COM RDF in Fig.~\ref{fig:tmp2b} (for how the 3-body COM RDF
is defined and presented we refer the readers to Appdix~\ref{app:tmp1}).
It can be seen that the 3-body corrlation is correctly reproduced
by the AdResS with TFI-IBI iterative correction in the
AT and HY I  regions.
That means the
the AT region is actually imbeded in a environment that up to the
3-body COM correlation is the same as the full atomistic reference
system.
In the HY II and HY III regions, the 3-body correlation deviate from the
full atomistic refrerence, when the first two molecules are near (the first
two columns). This means only reproducing the 2-body COM RDF \emph{does not}
indicate the correct reproduction of the 3-body COM RDF.
Also from Fig.~\ref{fig:tmp2a} and \ref{fig:tmp2b},
both the orientational pair correlation and the 3-body COM correlation
are short-ranged, therefore, we can safely say the AT region is only short-range
...
the pojection of Mol~3's position on the to the
