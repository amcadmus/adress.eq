...
Though the second order is numerically satisfying, higher level of accuracy can be systematically reached by requiring to match in the transition region higher \bluec{orders} of the probability distribution.
\bluec{Throughout the paper, when we mention the $i$-th order of a
  multi-body probability distribution, saying
...
\bluec{then the AT region is interacting with the $\HY$ region in an atomistic way, }
and \bluec{if we} assume the AT and $\HY$ regions are short-range correlated,
...
derive necessary conditions to enforce such an equivalence \bluec{in terms of orders of the configurational probability of the system; the lowest orders involve~\cite{rdfcorr}:}
...
Higher accuracy can be then systematically reached by enforcing the equivalence of higher \bluec{orders} of the distribution in the transition region.
...
\begin{align}\label{eqn:kappa-eq}
  \kappa_{\CG} &= \kappa_{\AT}.
\end{align}
Eq.\ref{eqn:kappa-eq} implies that the \bluec{COM-COM} radial distribution functions of the atomistic and coarse-grained region are matched \cite{han}.
...
so, instead, we study its marginal distributions up to the third order.
...
protocols and the definition of 3-body RDF, see the Appendix~\ref{app:2} and \ref{app:3}.
...
is embedded in an environment where not only the \bluec{COM-COM} structural properties but also finer structural
...
\end{figure}
  ...
  % This means that the RDFs in the
  % $\Delta$ region are not very accurate and, according to the theory developed in previous sections, one may not have good agreement for the considered \bluec{distributions}, in the atomistic region.
...
the projection of Mol~3's position onto the
