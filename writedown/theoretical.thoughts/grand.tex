...
Throughout the paper, when we mention the $i$-th order of a
  multi-body probability distribution, e.g.
...
then the AT region is interacting with the $\HY$ region in an atomistic way, 
and if we assume the AT and $\HY$ regions are short-range correlated,
...
derive necessary conditions to enforce such an equivalence in terms of orders of the configurational probability of the system; the lowest orders involve~\cite{rdfcorr}:
...
\begin{align}\label{eqn:kappa-eq}
  \kappa_{\CG} &= \kappa_{\AT}.
\end{align}
Eq.\ref{eqn:kappa-eq} implies that the COM-COM radial distribution functions of the atomistic and coarse-grained region are matched \cite{han}.
