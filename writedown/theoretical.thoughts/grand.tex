...
\bluec{Throughout the paper, when we mention the $i$-th order of a
  multi-body probability distribution
  $p(\vect r_1, \cdots, \vect r_N)$, we mean its $i$-th marginal
  distribution, which is defined by
  $p^{(i)}(\vect r_1, \vect r_2, \cdots, \vect r_i)
  = \int (\vect r_1, \cdots, \vect r_i, \vect r_{i+1}, \cdots, \vect r_N)
...
\bluec{
  Theoretically, the thermodynamic limit means fixing the density, the size of
  both the AT and CG regions go to infinite, and at the same time,
  the ratio of the size of AT and CG regions goes to 0.
  In pratice, we will never simulation an infinitly large system,
  and it is also not trivial to check the convergency of $p(N_1)$
  to a real grand canonical distribution~\eqref{eqn:p-2}
  with respect to
  the sizes of different subregions. Neverthless, we will compare the
  number distribution with the corresponding region of a full
  atomistic reference simulation, and show the $p(N_1)$
  in the AdResS system approximates corresponding
  subsystem of the full
  atomistic reference, which by definition converges to the grand canonical
...
system. In general this seems not be the case, \bluec{but it is possible to
derive necessary conditions to enforce such an equivalence in terms of orders of  configurational probability of the system;} the lowest orders involve
...
\begin{align}\label{eqn:kappa-eq}
  \kappa_{\CG} &= \kappa_{\AT}.
\end{align}
Eq.\ref{eqn:kappa-eq} implies that the center of mass-center of mass radial distribution functions of the atomistic and coarse-grained region are matched.
...
\bluec{ So far, there are two approaches to consider the
  accuracy of the  configuration
  distribution in AT region: 
  \begin{enumerate}
  \item
    The theoretical considerations proposed in  in Sec.~\ref{sec:p-1}:
    If the
    atomistic region is interacting with $\HY$ region in an
    atomistic way and the
    configuration of the $\HY$ region approximates 
    the full atomistic reference, then the configuration
    distributions in the atomistic region is of high accuracy.
    This argument is theoretically faciliate, because it
    allows us to raise necessary conditions
    and systematical ways to improve the
    accuracy of the configuration distribution in the atomistic region.
  \item From practical and numerical point of view,
    it is more convenient to directly consider the marginal
    distributions (density profile and RDFs) in the atomistic region.
    For most applications, the RDF accuracy up to the width of the lines
    is good enough.
    If this is the case,
    since the accuracy of the RDFs and $g^3$ in the
    HY I region is also very good, it is would be prefereable
    to consider HY I region as part of the AT region (Notice the
    weighting function $w(x)$ in HY I region has constant value 1, so
    this is actually the old AdResS way of defining AT region),
    because it wastes less in the $\HY$ region and provide better
    statistics in the AT region.
  \end{enumerate}
  % In the next section, we
  % will investigate the system of coupling the 
  % explicit water model with the WCA liquid model, as discussed
  % later, 
  % there is no easy way to systematically improve the configurational
  % accuracy of the $\HY$ region. Therefore, we use the second
  % way to study the configurational accuracy of the simulation,
  % i.e. the accuracy of distributions in the AT+HY I regions.
  Please note that if or not treat the region HY I as part of the
  AT region depends the accuracy required by the applications.
  Generally speaking, the structural accuracy in the AT region
  is better than the region HY I. In this example, they
  are equally good up to the resolution of the presented figures. Please
  also notice that actually there is no guarantee of the accuracy
  of H-O, H-H RDFs and $g^3$ in the HY I region. In the next numerical
  example, we do see difference in the AT and HY I regions.
}
...
\begin{figure}
  ...
  \caption{
    ...
    % Note that in lpot of $p(N_{1})$ the molecular number in the AT region,
    % $N_1$, is rescaled by the ensemble average: $N_1 / \langle N_1\rangle$.
  }
  \label{fig:wca-dist}
\end{figure}
...
% \begin{figure}
%   \centering
%   % \includegraphics[width=0.30\textwidth]{fig.grand/fig-rdf-wca-ex.eps}
%   % \includegraphics[width=0.30\textwidth]{fig.grand/fig-rdf-hhoh-wca-ex.eps}
%   \includegraphics[width=0.30\textwidth]{fig.grand/fig-pn-exhy.eps}
%   \caption{
%     The distributions of WCA AdResS in the AT region.
%     From left to right: the COM-COM RDF, the H-H and H-O RDFs, and
%     the distribution of number of molecules in the AT region.
...
% \end{figure}
  ...
  WCA AdResS is perfectly matching the all-atom reference system; satisfying agreement is found also in \bluec{the AT and HY I regions of}  the right plot of Fig.~\ref{fig:wca-den} which shows the molecular
  ...
  the COM-COM, H-O and H-H RDFs. In this case we want to investigate a ``worst case scenario'' and thus we did not apply the RDF corrective force in the transition region, thus the RDFs in the
  $\Delta$ region are not very accurate. It follows that according to the theory developed in previous sections, we cannot expect high consistency with respect to the second order in the distribution
  functions, i.e. the considered RDFs, in the atomistic region. However, results show that the
  WCA RDFs are nontheless very accurate, i.e. one cannot distinguish
  them from the full atomistic reference system and the standard AdResS
  within the width of the lines. This means that, from the numerical point of view, even only the work of the thermodynamic force, by allowing an exchange of particles under conditions of equilibrium of the chemical potential, is sufficient to assure the correct equilibrium of the atomistic region. The possible application of the corrective force on the RDF's in $\Delta$ in the case of liquid water is numerically not necessary, but in case of necessity it would anyway represent a systematic tool for improving accuracy. 
  ...
  % acceptable accuracy, only a
  % small discrepancy between the distributions is observed.
  % In this case, one must take into account that by matching the chemical potential, the 
  % theory in Sec.~\ref{sec:pn1} tells us that the distribution
  % can only be accurate at the first order, however the discrepancy should become less relevant as the system size increases and goes towards the thermodynamic limit (being the error of the second order).
  % In fact, the system set up in this case is a ``worst case scenario'': the
  % width of the AT, $\HY$ and CG regions are
  % 1.00~\textsf{nm}, 2.75~\textsf{nm},
  % 1.00~\textsf{nm}, respectively, which is far from corresponding to the thermodynamic
  % limit; however, as predicted above, by increasing the width of the CG  region to 16.00~\textsf{nm}, the discrepancy decreases and actually disappears.
  ...
  It shows that the $g^3$ in the atomistic region is essentially the same of the reference all atom simulation, thus a further numerically satisfying proof of accuracy provided by the filter of the transition region.
...
\bluec{
  If we have to be more careful about the structural accuracy in the AT region,
  then we consider in the way provided by the Sec.~\ref{sec:p-1}.
  The condition~\eqref{eqn:p-1-cond1} is statisfied by the thermodynamic
  force, and we are lucky that up all the pair-correlations are precise
  up to the resolution of the figures. Therefore, the
  necessary condition~\eqref{eqn:p-1-cond2} is \emph{numerically} statisfied.
  The difference is that we can achieve arbitrarily
  accurate condition \eqref{eqn:p-1-cond1} if we calculate the thermodynamic
  force careful enough, while for the RDF, we may see difference
  if the resolution of our simulation is higher (namely with longer
  sampling and smaller statistical error). Moreover, it may not be trivial
  to correct the COM RDF in the $\HY$ region, because
  the COM RDFs in the AT and CG regions are obviously different,
  it is not easy to acheive a atomistic COM RDF in between.
  Numerically, the all marginal distributions up to the thrid order are the
  same (limited by the resolution of figures)
  as the reference atomistic system in the AT region.
  If we are less careful and consider the AT and HY I regions together as
  a new atomistic region, then the configurational accuracy is only
  up to the pairwise interaction, because the $g^3$ in the HY II region
  is not accurate anymore.
  The theoretical $p(N_1)$ only has the 1st order of convergency speed, which
  is one order lower than the CG water model, however, in the numerical test,
  the fist two moments of the distribution and the distribution itself
  is satisfactory in the region AT+HY I.
}
