...
Since $\tilde q_{N_1}$ are positive definite, a basic
observation is that
\begin{align}
  \tilde q_{\bar N_1}
  \leq
  ...
  N \tilde q_{\bar N_1}. 
\end{align}
Due to the monotonicity of the logrithm, we have
\begin{align}
  \ln\tilde q_{\bar N_1}
  \leq
...
\end{align}
If $\ln N \ll \ln \tilde q_{\bar N_1}$ (the vailidity will be discussed
later), then 
\begin{align}
  \ln\sum_{N_1=1}^N \tilde q_{N_1}
  \approx
...
\end{align}
Where $A$ denotes the Helmhotz free energy. 
...
\begin{align}\label{eqn:condition}
  ...
  \ln Q_{\AT}(\bar N_1, V_1, T) + \ln Q_{\CG}(N - \bar N_1, V_3, T)
\end{align}
Generally this is true: $Q_{\CG}(N - \bar N_1, V_3, T)$
...
if the density there is the right density $\rho_0$, namely
$\hat N_1 = \rho_0V_1 = N - \rho_0 V_3$.
Notice, under the thermodynamic limit, $\hat N_1$ is consistent with
$\bar N_1$.
...
The ``h.o.t.'' standands for higher order terms (greater than 2)
of the density perturbation.
They can be safely neglected in apparence of
the first three terms of the expansion, so
we do not explicitly write them down.
...
$p_{\AT}(N_1)$ to the second leading order.
% This conclusion,
% compbined with the discussion on 
