...
\begin{abstract}
  ...
  work done by a thermodynamic force, that is a force derived on the basis of empirical thermodynamic considerations, \recheck{under reasonable approximations (always routinely occurring in simulation studies)} is formally equivalent to level the chemical
  potential difference between the different resolutions. From these results follows the main conclusion that,
  up to at least an accuracy of the second order with respect to the probability distribution of the system, the atomistic region exchanges
  molecules with the coarse-grained region in a Grand Canonical
  fashion.  We have also carried on numerical tests for the relevant case of liquid water at ambient conditions and show that the hypothesis done in our theoretical treatment are justified from the numerical point of view. Moreover, we show that in the atomistic region of AdResS even the three body correlation function of molecular centers of mass is the same as that of the equivalent region in a full atomistic simulation, thus strengthening the idea of AdResS as Grand Canonical-like framework. \recheck{Finally, a further numerical test is carried on: the coarse-grained molecules are substituted by a liquid of spheres interacting via the WCA potential which have no reference to the atomistic resolution. We show that also in this case in the atomistic region, due to the work of the filter of the transition region,  an accuracy up to the third order in the probability distribution is achieved. This is a further very strong numerical evidence, for the relevant case of liquid water, of the fact that indeed one can consider the AdResS set up as an effective Grand Canonical tool where the atomistic resolution is coupled to a source of energy and particles.}
\end{abstract}
...
\begin{figure}
  ...
  \caption{Local H-H and O-H $g(r)$'s.
    ...
    and the HY III subregion of $\Delta$, respectively. It can be seen that beyond 0.5 \textsf{nm} the functions go to one, that is particle beyond this distance are uncorrelated; this is fully consistent with our hypothesis of \recheck{ Eq.\ref{eqn:p-1-1}}.}
  \label{fig:tmp2a}
\end{figure}
...
as presented in the theoretical anlysis, corresponds to that of a grand canonical distribution  Eqn.~\eqref{eqn:p}.
...
been proved to be correct in Ref.~\cite{prlgc, rdfcorr} for the relevant case of liquid water and thus in this section, we want to test, with a numerical simulation, to what extent the configurational distribution \recheck{$p(\vect x_1) = \sum_{N_1}p(\vect x_1, N_1)$} is correct. 
...
is that the hypothesis of short-range influence of a region over another employed in \recheck{Eq.\ref{eqn:p-1-1}} is numerically justified and thus we can state that for the case of liquid water at ambient conditions the AT region is only short-range
...
\begin{figure}
  \centering
  \includegraphics[width=0.4\textwidth]{fig.grand/fig-rho-wca.eps}
  \includegraphics[width=0.4\textwidth]{fig.grand/fig-count-wca.eps}
...
\begin{figure}
  \centering
  \includegraphics[width=0.4\textwidth]{fig.grand/fig-rdf-wca-ex.eps}
  \includegraphics[width=0.4\textwidth]{fig.grand/fig-rdf-hhoh-wca-ex.eps}
  \caption{
  }
  \label{fig:wca-rdf}
\end{figure}
...
\bluec{
In the WCA AdResS, the ``coarse-grained water'' potential is given by
\begin{align}
  U(r) = 4\varepsilon
  \big[
...
the hard core of WCA is roughly the same as the structure-based coarse-grained
water model.
